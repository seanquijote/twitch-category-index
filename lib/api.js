/**
 * @file lib/api.js
 * @description Twitch Helix API client for fetching the complete games list.
 * Handles cursor-based pagination and rate-limiting transparently, returning
 * all raw game objects as a single flat array.
 *
...
/** @import { RawGame, GamesResponse } from './types.js' */

/**
 * Fetches every game from the Twitch Helix `GET /games/top` endpoint by
 ...
 * @param {function} [_getFn]    - HTTP GET function to use; defaults to `httpsGet`.
 *                                 Signature: `(url: string, headers: object) => Promise<GamesResponse>`
 * @returns {Promise<RawGame[]>} - All raw game objects across all pages
 * @throws {Error}               - If any individual page request fails
 *
 * @example
 * const token = await getAccessToken(clientId, clientSecret);
 * const games = await fetchAllGames(clientId, token);
 * console.log(games.length); // → ~58000
 */
export async function fetchAllGames(clientId, accessToken, _getFn = httpsGet) {
  const headers = {
    "Client-ID": clientId,
  ...
  do {
    ...
    const res = /** @type {GamesResponse} */ (await _getFn(url, headers));
    const data = res.data ?? [];
    ...
    process.stdout.write(
      `\r   Page ${page} — ${games.length} games fetched so far...`,
    );

    if (cursor) await sleep(RATE_LIMIT_DELAY);
  } while (cursor);

  console.log(`\n✅ Done fetching. Total from API: ${games.length} games.`);
  return games;
}
